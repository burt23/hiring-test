...
### Available commands
* `npm run dev` to serve a development version of the project with a client AND server

### Objectives
1. Create an internal api endpoint that fetches from the following external resources and returns a transformed dataset. Use the response as the new source for mattress data.
...
2. Create a "CartWidget" that toggles when you click the cart icon. 
    - CartWidhet should have:
        * label
        * price
